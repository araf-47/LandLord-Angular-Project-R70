...
import { MockDataService, Unit, nextId } from '../../../core/mock-data.service';
  ...
  save(): void {
    ...
    if (this.editing) {
      this.data.units.update((list) =>
        list.map((u) => (u.id === this.unitId ? { ...u, unitNumber: this.unitNumber, rent: this.rent!, status: this.status } : u))
      );
    } else {
      this.data.units.update((list) => [
        ...list,
        { id: nextId('u'), propertyId: this.propertyId, unitNumber: this.unitNumber, rent: this.rent!, status: this.status },
      ]);
    }
